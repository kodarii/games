...
### IGDB OAuth token cache (added 2026-05-20 post-research — same-pattern fix per D-07)
- **D-21:** `igdb_oauth_token` is currently a singleton row (`SINGLETON_ID = 1`, no `user_id`) — when two users have IGDB configured, the second user's token mint overwrites the first user's row. This is the same conceptual bug as `IgdbChainHolder` one layer deeper and MUST be fixed in this phase per D-07. **Approach: in-memory per-user token cache inside `IgdbTokenStore`** (`Map<string, IgdbAccessToken>` keyed on `userId`, lazy mint via Twitch OAuth, no DB write). The existing `igdb_oauth_token` table is no longer the source of truth — tokens live in memory only. Refresh on `401` from IGDB or on TTL expiry happens per-user naturally. Survives no restarts; first request after restart re-mints via Twitch (~one HTTP round-trip, already happens on first use today).
- **D-22:** **No DB schema change** preserves D-18. The `igdb_oauth_token` table is retired in-code: writes stop, reads are removed. Drop-table migration is out of scope for Phase 6 (the orphan table is harmless — append to `CONCERNS.md` as a future cleanup). Document the retirement in the registry/token-store files (TSDoc on `IgdbTokenStore`).
- **D-23:** `IgdbTokenStore` API gains `userId` on every public method (`get(userId)`, `invalidate(userId)`). The in-flight refresh dedupe (`inflightRefresh` per `apps/api/src/infrastructure/igdb/igdb-token-store.ts:96-105`) becomes keyed on `userId`, not global.
