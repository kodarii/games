...
import { usePlatformsQuery, useUpdateGameMutation } from '@/lib/queries';
...
export function GameForm({
  ...
  const { draft, set, toPayload } = useGameDraft(initialGame);
  ...
  const canSubmit = Boolean(draft.title.trim() && draft.developer.trim() && draft.platform);

  const onSubmit = () => {
    const payload = toPayload({ kind: mode });
    const successPath = isWishlist ? '/wishlist' : '/games';

    updateMutation.mutate(
      { id: initialGame.id, input: payload },
      { onSuccess: (g) => navigate(`${successPath}/${g.id}`) },
    );
  };

  const headerIcon = <Icon.gamepad size={15} className="text-white" />;
  const headerTitle = 'Edit Game';
  const headerDescription = 'Update details for this game.';
  const breadcrumbLast = initialGame.title;
  const submitLabel = 'Save Changes';
  const parentLabel = isWishlist ? 'Wishlist' : 'Games';
  const parentPath = isWishlist ? '/wishlist' : '/games';
  const errorMessage = updateMutation.error?.message ?? null;
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr]">
            <div className="flex items-center justify-center p-5 lg:border-r lg:border-apex-line-5 lg:p-6">
              <div className="w-full max-w-[260px] lg:max-w-none">
                <div className="flex flex-col items-stretch gap-4">
                  <GameCover name={draft.title} color={draft.coverColor} src={draft.coverImage} />
                  <div>
            ...
            <div className="flex flex-col">
              ...
              <div className="border-t border-apex-line-5 p-5 lg:p-6">
                <SectionHeader
                  title="Game"
                  description="Search the title — we'll try to match it to IGDB."
                />
                <FormFieldRow cols={2}>
              ...
              {!isWishlist && (
                <div className="border-t border-apex-line-5 p-5 lg:p-6">
                  <SectionHeader
                    title="Status"
                    description="Set the current status for this game."
                  />
                  <PillSelect
                    value={draft.status ?? 'Backlog'}
                    options={STATUS_OPTS}
                    onChange={(v: GameStatus) => set('status', v)}
                  />
                </div>
              )}
            </div>
      ...
      <FormFooter>
        ...
        <FormSubmitButton disabled={!canSubmit || updateMutation.isPending} onClick={onSubmit}>
          {submitLabel}
        </FormSubmitButton>
      </FormFooter>
